...
class MongoDbEvaluator(impl: MongoDbEvaluatorImpl[({type λ[α] = StateT[Task, SequenceNameGenerator.EvalState,α]})#λ]) extends Evaluator[WorkflowOp] {
  def execute(physical: WorkflowOp, out: Option[Path]): Task[Path] = for {
...
object MongoDbEvaluator {
  ...
  def toJS(physical: WorkflowOp, out: Option[Path]): EvaluationError \/ String = {
...
trait MongoDbEvaluatorImpl[F[_]] {
  ...
  def execute(physical: WorkflowOp, out: Option[Path])(implicit MF: Monad[F]): F[Path] = {
    ...
    val outColl = out.map(Collection.fromPath(_)).sequenceU
    outColl.fold(
      ...
      outColl => for {
                    ...
                    _   <- temps.collect {
                              case tmp @ Col.Tmp(coll) => if (tmp != dstCol) executor.drop(coll)
                                                          else outColl.map(executor.rename(coll, _)).getOrElse(().point[F])
                            }.sequenceU
                  } yield dstCol match { case Col.User(coll) => coll.asPath; case Col.Tmp(coll) => out.getOrElse(coll.asPath) }
    )
  }
}
