...
object Selector {
  ...
  case class In(rhs: Bson.AnonElem) extends SimpleSelector("$in") with Comparison
  ...
  case class Nin(rhs: Bson.AnonElem) extends SimpleSelector("$nin") with Comparison

  sealed trait Logical extends Selector
  case class Or(conditions: List[Selector]) extends SimpleSelector("$or") with Logical {
    protected def rhs = Bson.AnonElem(conditions.map(_.bson))
  }
  ...
  case class Mod(divisor: Int, remainder: Int) extends SimpleSelector("$mod") with Evaluation {
    protected def rhs = Bson.AnonElem(Bson.Int32(divisor) :: Bson.Int32(remainder) :: Nil)
  }
  ...
  case class GeoWithin(geometry: String, coords: List[List[(Double, Double)]]) extends SimpleSelector("$geoWithin") with Geospatial {
    protected def rhs = Bson.Doc(Map(
      "$geometry" -> Bson.Doc(Map(
        ...
        "coordinates" -> Bson.AnonElem(coords.map(v => Bson.AnonElem(v.map(t => Bson.AnonElem(Bson.Dec(t._1) :: Bson.Dec(t._2) :: Nil)))))
      ))
    ))
  }
  ...
  case class Near(lat: Double, long: Double, maxDistance: Double) extends SimpleSelector("$near") with Geospatial {
    protected def rhs = Bson.Doc(Map(
      "$geometry" -> Bson.Doc(Map(
        ...
        "coordinates" -> Bson.AnonElem(Bson.Dec(long) :: Bson.Dec(lat) :: Nil)
      ))
    ))
  }
  ...
  sealed trait AnonElem extends Selector
  case class ContainsAll(selectors: Seq[Selector]) extends SimpleSelector("$all") with AnonElem {
    protected def rhs = Bson.AnonElem(selectors.map(_.bson))
  }
  case class ExistsElemMatch(selector: Selector) extends SimpleSelector("$elemMatch") with AnonElem {
    protected def rhs = selector.bson
  }
  case class HasSize(size: Int) extends SimpleSelector("$size") with AnonElem {
  ...
  case class Slice(skip: Int, limit: Option[Int]) extends SimpleSelector("$") with Proj {
    protected def rhs = (limit.map { limit =>
      Bson.AnonElem(Bson.Int32(skip) :: Bson.Int32(limit) :: Nil)
