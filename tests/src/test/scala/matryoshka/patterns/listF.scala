...
class ListFSpec extends Specification with ScalaCheck with CheckAll with AlgebraChecks {
  ...
  checkAlgebraIsoLaws("ListF ⇔ List", ListF.listIso[Int])(imp, Arbitrary(Gen.listOf(Arbitrary.arbInt.arbitrary)), imp, imp, imp)
  checkAlgebraIsoLaws("ListF ⇔ Seq", ListF.seqIso[Int])(imp, Arbitrary(Gen.listOf(Arbitrary.arbInt.arbitrary).map(_.toSeq)), imp, Equal.equalA[Seq[Int]], imp)
}
