...
trait Planner[PhysicalPlan] {
  ...
  def queryPlanner(showNative: PhysicalPlan => (String, Cord))(implicit RA: RenderTree[PhysicalPlan]):
    ...
    (for {
      ...
      tree       <- withTree("Annotated Tree")(AllPhases(tree(select)).disjunction.leftMap(e => ManyErrors(e.map(ESemanticError))))
      tree       <- withTree("Annotated Tree (variables substituted)")(Variables.substVars[SemanticAnalysis.Annotations](tree, _._2, req.variables).leftMap(ESemanticError))
