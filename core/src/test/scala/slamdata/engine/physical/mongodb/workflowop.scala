  ...
  "smart constructors" should {
    ...
    "flatten project into group/unwind" in {
      val given = chain(
        ...
        $group(
          Grouped(ListMap(
            BsonField.Name("value") -> $push($var("rIght")))),
          -\/($var("lEft"))),
        $unwind(DocField(BsonField.Name("value"))),
        $project(Reshape(ListMap(
          BsonField.Name("city") -> -\/($var("value", "city")))),
          IncludeId))
      ...
      val expected = chain(
        ...
        $group(
          Grouped(ListMap(
            BsonField.Name("city") -> $push($var("rIght", "city")))),
          -\/($var("lEft"))),
    ...
    "traverse `Include`" in {
      ...
      chain($read(Collection("db", "zips")),
        $project(Reshape(ListMap(
          BsonField.Name("bar") ->
            -\/($divide($var("baz"), $literal(Bson.Int32(92)))))),
          IncludeId))
    }
  ...
  "task" should {
    ...
    "always pipeline unconverted aggregation ops" in {
      ...
      task(Crystallized(chain(
        ...
        $group(
          Grouped(ListMap(
            BsonField.Name("__sd_tmp_1") -> $push($var("lEft")))),
          -\/($literal(Bson.Int32(1)))),
        $project(Reshape(ListMap(
          BsonField.Name("a")      -> -\/($include()),
          BsonField.Name("b")      -> -\/($include()),
          BsonField.Name("equal?") -> -\/($eq($var("a"), $var("b"))))),
          IncludeId),
        $match(Selector.Doc(
      ...
      beTree[WorkflowTask](
        PipelineTask(ReadTask(Collection("db", "zips")),
          List(
            ...
            $Project((),
              Reshape(ListMap(
                BsonField.Name("a")      -> -\/($var("a")),
                BsonField.Name("b")      -> -\/($var("b")),
                BsonField.Name("equal?") -> -\/($eq($var("a"), $var("b"))))),
              IncludeId),
            $Match((),
              Selector.Doc(
