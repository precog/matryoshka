  ...
  "smart constructors" should {
    ...
    "flatten project into group/unwind" in {
      val given = chain(
        ...
        $group(
          Grouped(ListMap(
            BsonField.Name("value") -> $push($("rIght")))),
          -\/($("lEft"))),
        $unwind(DocField(BsonField.Name("value"))),
        $project(Reshape(ListMap(
          BsonField.Name("city") -> -\/($("value", "city")))),
          IncludeId))
      ...
      val expected = chain(
        ...
        $group(
          Grouped(ListMap(
            BsonField.Name("city") -> $push($("rIght", "city")))),
          -\/($("lEft"))),
    ...
    "traverse `Include`" in {
      ...
      chain($read(Collection("db", "zips")),
        $project(Reshape(ListMap(
          BsonField.Name("bar") ->
            -\/($divide($("baz"), $literal(Bson.Int32(92)))))),
          IncludeId))
    }
  ...
  "task" should {
    ...
    "always pipeline unconverted aggregation ops" in {
      task(chain(
        ...
        $group(
          Grouped(ListMap(
            BsonField.Name("__sd_tmp_1") -> $push($("lEft")))),
          -\/($literal(Bson.Int32(1)))),
        $project(Reshape(ListMap(
          BsonField.Name("a")      -> -\/($include()),
          BsonField.Name("b")      -> -\/($include()),
          BsonField.Name("equal?") -> -\/($eq($("a"), $("b"))))),
          IncludeId),
        $match(Selector.Doc(
      ...
      beTree[WorkflowTask](
        PipelineTask(ReadTask(Collection("db", "zips")),
          List(
            $Group((), Grouped(ListMap()), -\/($literal(Bson.Null))),
            $Project((),
              Reshape(ListMap(
                BsonField.Name("a")      -> -\/($("a")),
                BsonField.Name("b")      -> -\/($("b")),
